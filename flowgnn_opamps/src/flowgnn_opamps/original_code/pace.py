...
class PACE_VAE(nn.Module):
    ...
    def _prepare_features(self,glist,mem_len=None):
        ...
        """
        bsize = len(glist)
        node_feature = torch.zeros(bsize,self.max_n,self.nvt + 1).to(self._get_device()) # we take one-hot encoding as the initial features
        ...
        for i in range(bsize):
            ...
            if num_node < self.max_n:
                ...
                nfeat += [0.] * (self.max_n - num_node)
                ptype += [self.END_TYPE] * (self.max_n - num_node)

            nfeat = torch.FloatTensor(nfeat).view(-1,1).to(self._get_device())
            ...
            node_feature[i,:,:] = torch.cat([ntype_one_hot, nfeat], dim=1) # the 'extra' nodes are padded with the zero embeddings
            # position one-hot
